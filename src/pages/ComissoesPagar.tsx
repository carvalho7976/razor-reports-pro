...
import { User, CheckCircle, Printer, CreditCard, HandCoins, Handshake } from "lucide-react";
...
const initialData: Comissao[] = [
  {
    id: 1,
    profissional: "Cesar",
    totalComissoes: 4956.2,
    totalAdiantamentos: 300,
    totalPagar: 4656.2,
    status: "Em aberto",
  },
  {
    ...
    totalComissoes: 3737.97,
    totalAdiantamentos: 20,
    totalPagar: 3717.97,
    status: "Em aberto",
  },
  {
    ...
    totalComissoes: 14.63,
    totalAdiantamentos: 60,
    totalPagar: -45.37,
    status: "Pago",
  },
...
export default function ComissoesPagar() {
  ...
  const data = useMemo(
    () => allData.filter((d) => (tab === "em_aberto" ? d.status === "Em aberto" : d.status === "Pago")),
    [tab, allData],
  );
  ...
  const bulkPrint = (indices: number[]) => {
    toast({
      title: `Imprimir relatório de ${indices.length} funcionário(s)`,
  ...
  const tabs: TabDef[] = [
    {
      label: "Em aberto",
      value: "em_aberto",
      count: allData.filter((d) => d.status === "Em aberto").length,
      color: "destructive",
    },
