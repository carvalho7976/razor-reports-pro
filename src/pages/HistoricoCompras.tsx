...
interface Produto { nome: string; valor: number; quantidade: number; total: number; }
interface CompraResumida { id: number; data: string; funcionario: string; valor: number; desconto: number; total: number; produtos: Produto[]; }
...
export default function HistoricoCompras() {
  ...
  const columnsResumido: Column<any>[] = [
    ...
    { key: "desconto", label: "Desconto", align: "right", render: (v: number) => R$(v) },
    {
      ...
      render: (v: number) => <span style={{ color: "#00c5b4" }} className="font-medium">{R$(v)}</span>,
    },
  ];
