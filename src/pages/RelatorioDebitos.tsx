...
const buildCards = (filtered: Debito[]): SummaryCard[] => [
  { label: "Total em Aberto", value: R$(filtered.filter(d => d.status === "Em aberto").reduce((s, r) => s + r.valor, 0)), icon: <CreditCard className="h-4 w-4" />, size: "wide", color: "red" },
  { label: "Total Pago", value: R$(filtered.filter(d => d.status === "Pago").reduce((s, r) => s + r.valor, 0)), icon: <CreditCard className="h-4 w-4" />, size: "wide", color: "green" },
];
...
export default function RelatorioDebitos() {
  ...
  const columns: Column<Debito>[] = [
    { key: "cliente", label: "Cliente", pinned: true, render: (v, row) => (
  ...
  return (
    <AppLayout>
      ...
      />
      <YouTubeModal open={aulaOpen} onClose={() => setAulaOpen(false)} videoUrl="https://www.youtube.com/watch?v=dQw4w9WgXcQ" title="Aula - Relatório de Débitos" />
    </AppLayout>
  );
}
