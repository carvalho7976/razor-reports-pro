...
import { TextField } from "@/components/FormModal";
          ...
          <div className="flex flex-col gap-5">
            ...
            <SectionBlock id="dados" title="Dados">
              <div className="grid gap-4">
                {/* ↓ CORREÇÃO: grid sem md:grid-cols-2 para evitar sobreposição dos selects em telas médias */}
                <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-[minmax(0,1.8fr)_minmax(140px,1fr)_minmax(0,1.1fr)_minmax(0,1.3fr)]">
                  <TextField
                    label="Nome do Plano"
                    value={nome}
                    onChange={setNome}
                  ...
                  {/* Recorrência — Select nativo com w-full para não vazar */}
                  <div className="grid min-w-0 gap-0.5">
                    <label className="text-[13px] font-semibold text-foreground">Recorrência</label>
                    <Select value={recorrencia} onValueChange={setRecorrencia}>
                      <SelectTrigger className="h-10 w-full rounded-lg border border-info/50 bg-card px-3 text-sm focus:ring-2 focus:ring-info/40">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        {recorrenciaOptions.map((o) => (
                  ...
                  {/* Forma de Pagamento — Select nativo com w-full para não vazar */}
                  <div className="grid min-w-0 gap-0.5">
